...
function upload(options, { key, data: path, type }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    qiniu.zone.getZoneInfo(qiniu.conf.ACCESS_KEY, bucket, function (err, cZoneInfo, cZoneExpire) {
      if (err) {
        reject(err);
        return;
      }
      // update object
      config.zone = cZoneInfo;
      config.zoneExpire = cZoneExpire + parseInt(Date.now() / 1000);
